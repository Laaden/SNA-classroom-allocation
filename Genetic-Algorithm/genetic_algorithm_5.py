...
# pick the first column containing 'cluster' (excluding student_id)
cluster_candidates = [
    c for c in cols
...
if cluster_candidates:
    ...
    # fallback: first non-ID, non-index column
    fallback = [
        c for c in cols
        if c.lower() != sid_col.lower() and "index" not in c.lower()
    ]
    if not fallback:
        raise ValueError(f"Columns found: {cols}. Couldn't detect a cluster column.")
    cluster_col = fallback[0]

# rename for consistency and coerce types
...
# merge cluster labels with performance scores
df = (
    df_clusters[[sid_col]]
      .merge(
...
# evaluation: std-dev of sizes ↓, worst-class mean ↑
...
# ——— 5) Run GA, persist results & edges ———
def main(seed=42):
    np.random.seed(seed)
    # initialise and select
    ...
    pd.DataFrame({sid_col:      student_ids,
                  "cluster_label": best
    ...
    # 5b) write optimized clusters back to MongoDB
    ...
    cluster_coll.insert_many(docs)
    print(f"Inserted {len(docs)} docs into '{cluster_coll.name}'.")

    # 5c) export edges (Participant-ID → Target School Activities)
    EDGE_FIELD = "Target School Activities"
    LABEL      = "School Activities"
    edges_coll = db["result_edges_info"]
    edges_coll.delete_many({})
    edge_docs = []
    for d in db.sna_student_raw.find({}, {"_id": 0, "Participant-ID": 1, EDGE_FIELD: 1}):
        pid = d.get("Participant-ID")
        to  = d.get(EDGE_FIELD)
        if pid is None or to is None:
            continue
        edge_docs.append({"from": int(pid), "to": int(to), "label": LABEL})
    if edge_docs:
        edges_coll.insert_many(edge_docs)
    print(f"Inserted {len(edge_docs)} edges into '{edges_coll.name}'.")
